...
function mostrar (fecha, callback){
    mysql.query(`SELECT controlactivi.idcontrolactivi, controlactivi.responsables, actividades.actividad, tiempo_actividades.status as estatusT, asigactivi.status as estatusA, 
                ...
                where controlactivi.fecha = "${fecha}" && asigactivi.status != "INACTIVO"`, function(error,respuesta){
        if(error){
            callback(null,{
                mensaje:  error
            })
